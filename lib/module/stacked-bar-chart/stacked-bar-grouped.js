...
class StackedBarGrouped extends PureComponent {
    constructor(...args) {
        ...
        _defineProperty(
            ...
            (x0, y0, x1, y1, borderRadius, showTopBorder, showBottomBorder) => {
                const commands = []
                commands.push({
                    marker: 'M',
                    values: [x0, y0],
        ...
        _defineProperty(this, 'commandsToSvgPath', (commands) =>
            commands
                .map((command) => ''.concat(command.marker, ' ').concat(command.values.join(',')))
                .join(' ')
                .trim()
        )
    }
    ...
    calcExtent(values) {
        const { gridMax, gridMin } = this.props // One more merge for stacked groups
    ...
    calcIndexes() {
        const { data } = this.props // Must return an array with indexes for the number of groups to be shown
